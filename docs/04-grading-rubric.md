...
```
The grader looks at the **INTEGRITY:** line. `VERIFIED` means the script is untampered. `*** MISMATCH ***` means the student edited the check script — this is an academic-integrity violation and the entire submission is graded **0** under [Criterion 4](#criterion-4--submission-hygiene--integrity), regardless of how the rest looks. `UNKNOWN` is treated as a re-record-and-resubmit, not a violation — the script's own `UNKNOWN` message tells the student which cause to fix (the VM couldn't reach GitHub, the script's path is missing from `CHECKSUMS.txt`, or `sha256sum`/`shasum` isn't installed). The grader will ask for a re-run once `VERIFIED` can be shown.
